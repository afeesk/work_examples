...
###                   ###
#########################
# define training control, 10 fold
train_control_CV <- trainControl(method="cv", number=10)
model_gbm_CV <- train(customer_train_clean_3[, -1], 
                   customer_train_clean_3$transactionRevenue, 
                   trControl=train_control_CV, method = 'gbm')
model_gbm_CV
#save(model_gbm_CV, file = "model_gbm_CV.RData")
#load("model_gbm_CV.RData")

predict_gbm_CV_test <- predict(model_gbm_CV, newdata = customer_test_clean_3)
...
write.csv(submision_1, file = paste0(getwd(), "/", "submissions/submission_1.csv"), row.names = FALSE)
...
write.csv(submision_2, file = paste0(getwd(), "/", "submissions/submission_2.csv"), row.names = FALSE)
